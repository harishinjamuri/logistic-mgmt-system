...
file name           : route-report.php
database connection : buspassdatabase
table               : student
purpose             :
             The functionality of this page is to generate report based on a route
...
<title>Logistics Mgt. | Stud Route Reports</title>
...
<p style="color:#116EB0;font-size:14px; "><a href="adminhome.php">Home > </a>Generate > Student Reports ><a href="route-report.php"> Route Report</a></p><br>
 ...
 <!-- script starts for date picker -->
  ...
  <!-- script ends -->
    ...
    <b>Select Route</b> &nbsp;&nbsp;
    <select name="routeno" id="routeno" style="width:250px;border:1px solid black;height:30px;" class="form-control"  />
...
//  echo $_GET['date1'];
  ...
  if(isset($_GET['page']) || isset($_GET['routeno']) )
  { if(isset($_GET['routeno'])){
   ...
   // echo $dt1." ".$dt2;
      ...
      /* when date is picked */
       $query = "SELECT Receipt_Number,HallticketNumber,Name,Route_no,Boarding_point,Course,Department_Name,Year,Total,Due,Verifiedby,Remarks FROM student where Date_of_Payment BETWEEN '$dt1' AND '".date('Y-m-d',strtotime('+1 day', strtotime($dt2)))."' AND Route_no = '".$_GET['routeno']."' ORDER BY Date_of_Payment DESC";
	 ...
	 /* when date is not picked and branch other than pharmacy is picked  */	
	  ...
	  if($_GET['routeno'] != "")	
	  {   $query = "SELECT Receipt_Number,HallticketNumber,Name,Route_no,Boarding_point,Course,Department_Name,Year,Total,Due,Verifiedby,Remarks FROM student where Route_no = '".$_GET['routeno']."' ORDER BY Date_of_Payment DESC";
		  $search_result = mysqli_query($conn, $query);
	  }
    ...
    //echo $dt1." ".$dt2;
	   ...
	   $st1 = "SELECT * FROM student where Date_of_Payment BETWEEN '".$dt1."' AND '".date('Y-m-d',strtotime('+1 day', strtotime($dt2)))."' AND  Route_No = '".$_GET['routeno']."' ORDER BY Date_of_Payment DESC LIMIT ".$this_page_first_result.",".$results_per_page ;
	  ...
	  if($_GET['routeno'] != "")	{
	  $st1 = "SELECT * FROM student where Route_No = '".$_GET['routeno']."' ORDER BY Date_of_Payment DESC LIMIT ".$this_page_first_result.",".$results_per_page ;
	  $search_result1 = mysqli_query($conn,$st1 );
	 // echo $st1;
	  }	 
  ...
  <?php 
	 ...
	 <h6 style="color:red;margin-left:100px;">No Data Found.</h6>
	  ...
	  ?>
	  <h6 style="color:red;margin-left:100px;">Please select Route and click on Go.</h6>
	  <?php
  }  
...
<a href="<?php $page = 1;  echo basename($_SERVER['PHP_SELF']).'?page=' . $page . '&datepicker='.$dt1.'&datepicker1='.$dt2.'&routeno='.$_GET['routeno'].' ';?>" style="text-decoration: none;color:white !important;<?php if ($_GET['page'] == 1 || $number_of_pages == 1){ echo 'pointer-events:none;background-color:rgba(17,110,176,0.5) !important;';} else{ echo 'background-color:#116eb0 !important;'; }?> " ><<</a>
...
for ($page=$pages;$page <= $number_of_pages && $page<= $pages + 10;$page++) { ?> 
 ...
 <!-- basename is for getting the name of thee file -->
  ...
  <!-- anchor tag which takes to next page in pagination  -->
	 ...
	 <a href="<?php $page = $_GET['page'] + 1; echo basename($_SERVER['PHP_SELF']).'?page=' . $page . '&datepicker='.$dt1.'&datepicker1='.$dt2.'&routeno='.$_GET['routeno'].' ';?>" style="margin-left:10px;text-decoration: none;color:white !important;<?php if ($_GET['page'] == $number_of_pages || $number_of_pages == 1) {echo 'pointer-events:none;background-color:rgba(17,110,176,0.5) !important;'; }else{ echo 'background-color:#116eb0 !important;' ; }?>">></a>   
