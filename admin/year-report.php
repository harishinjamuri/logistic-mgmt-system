...
file name           : year-report.php
database connection : buspassdatabase
table               : student
purpose             :
             The functionality of this page is to generate report based on a Date_of_Payment
...
<title>Logistics Mgt. | Stud Year Reports</title>
...
<p style="color:#116EB0;font-size:14px; "><a href="adminhome.php">Home > </a>Generate > Student Reports ><a href="branch-report.php"> Year Report</a></p><br>
 ...
 <!-- script starts for date picker -->
  ...
  <!-- script ends -->
    ...
    <b>Select Year</b> &nbsp;&nbsp;
    <select class="form-control" id="year" style="height:30px;width:250px;border:1px solid black;"  name="year"  required />
...
height:30px;color:white;background-color: #116eb0;">
  ...
  <script>
   ...
   function date2(){
	 ...
	 // alert($.datepicker.formatDate("dd-mm-yy", date1));
   }
...
//  echo $_GET['date1'];
  ...
  if(isset($_GET['page']) || isset($_GET['year']) )
  {
   ...
   // echo $dt1." ".$dt2;
      ...
      /* when date is picked */
       $query = "SELECT Receipt_Number,HallticketNumber,Name,Route_no,Boarding_point,Course,Department_Name,Year,Total,Due,Verifiedby,Remarks FROM student where Date_of_Payment BETWEEN '$dt1' AND '".date('Y-m-d',strtotime('+1 day', strtotime($dt2)))."' AND Year = '".$_GET['year']."' ORDER BY Date_of_Payment DESC";
	 ...
	 /* when date is not picked and branch other than pharmacy is picked  */	
	  ...
	  if($_GET['year'] != "")	
	  {   $query = "SELECT Receipt_Number,HallticketNumber,Name,Route_no,Boarding_point,Course,Department_Name,Year,Total,Due,Verifiedby,Remarks FROM student where Year = '".$_GET['year']."' ORDER BY Date_of_Payment DESC";
		  $search_result = mysqli_query($conn,$query );
	  }
    ...
    //echo $dt1." ".$dt2;
	   ...
	   $st1 = "SELECT * FROM student where Date_of_Payment BETWEEN '".$dt1."' AND '".date('Y-m-d',strtotime('+1 day', strtotime($dt2)))."' AND Year = '".$_GET['year']."' ORDER BY Date_of_Payment DESC LIMIT ".$this_page_first_result.",".$results_per_page ;
	  ...
	  if($_GET['year'] != "")	{
	  $st1 = "SELECT * FROM student where Year = '".$_GET['year']."' ORDER BY Date_of_Payment DESC LIMIT ".$this_page_first_result.",".$results_per_page ;
	  $search_result1 = mysqli_query($conn,$st1 );
	 // echo $st1;
	  }	 
  ...
  <?php } ?>
   ...
   <p style="color:#116EB0">Latest on Top</p>
      <table  class="table " style="text-align:center;border:1px solid black;margin-top:10px;"  width="100vw">
        <tr style=" background: rgba(17,110,176,0.9);color:white;">
		     <th>#</th>
			<th>Receipt</th>
			<th>Hallticket</th>
  ...
  else if($_GET['year'] == "SELECT"){
	  ?>
	 <h6 style="color:red;margin-left:100px;">Please select Year and close on go.</h6>
...
<a href="<?php $page = 1;  echo basename($_SERVER['PHP_SELF']).'?page=' . $page . '&datepicker='.$dt1.'&datepicker1='.$dt2.'&year='.$_GET['year'].' ';?>" style="text-decoration: none;color:white !important;<?php if ($_GET['page'] == 1 || $number_of_pages == 1){ echo 'pointer-events:none;background-color:rgba(17,110,176,0.5) !important;';} else{ echo 'background-color:#116eb0 !important;'; }?> " ><<</a>
...
for ($page=$pages;$page <= $number_of_pages && $page<= $pages + 10;$page++) { ?> 
 ...
 <!-- basename is for getting the name of thee file -->
  ...
  <!-- anchor tag which takes to next page in pagination  -->
	 ...
	 <a href="<?php $page = $_GET['page'] + 1; echo basename($_SERVER['PHP_SELF']).'?page=' . $page . '&datepicker='.$dt1.'&datepicker1='.$dt2.'&year='.$_GET['year'].' ';?>" style="margin-left:10px;text-decoration: none;color:white !important;<?php if ($_GET['page'] == $number_of_pages || $number_of_pages == 1) {echo 'pointer-events:none;background-color:rgba(17,110,176,0.5) !important;'; }else{ echo 'background-color:#116eb0 !important;' ; }?>">></a>   
