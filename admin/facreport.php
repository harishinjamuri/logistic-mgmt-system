...
<title>Logistics Mgt. | Employee Reports</title>
...
<p style="color:#116EB0;font-size:14px; "><a href="adminhome.php">Home > </a>Generate><a href="facreport.php"> Employee Report</a></p><br>
...
//  echo $_GET['date1'];
  ...
  if(isset($_GET['page']) || isset($_GET['branch']) )
  {
   ...
   // echo $dt1." ".$dt2;
      ...
      /* when date is picked */
	   ...
	   $query = "SELECT  `Employee_id`, `Name`, `Designation`, `Department`,`Mobile_Number`,`Boarding_point`, `Route_no`, `Fee`, `Due`,`Verifiedby`,Remarks FROM `faculty` WHERE Date_of_Payment BETWEEN '$dt1' AND '".date('Y-m-d',strtotime('+1 day', strtotime($dt2)))."' ORDER BY Date_of_Payment DESC";
	 ...
	 /* when date is not picked and branch other than pharmacy is picked  */	
	  ...
	  $query = "SELECT `Employee_id`, `Name`, `Designation`, `Department`,`Mobile_Number`,`Boarding_point`, `Route_no`, `Fee`, `Due`,`Verifiedby`,Remarks FROM `faculty` ORDER BY Date_of_Payment DESC";
	  $search_result = mysqli_query($conn, $query);
    ...
    //echo $dt1." ".$dt2;
	   ...
	   $st1 = "SELECT `Employee_id`, `Name`, `Designation`, `Department`,`Mobile_Number`,`Boarding_point`, `Route_no`, `Fee`, `Due`,`Verifiedby`,`Receipt_Number` FROM `faculty` WHERE Date_of_Payment BETWEEN '$dt1' AND '".date('Y-m-d',strtotime('+1 day', strtotime($dt2)))."' ORDER BY Date_of_Payment DESC LIMIT ".$this_page_first_result.",".$results_per_page ;
	  ...
	  {
	  $st1 = "SELECT `Employee_id`, `Name`, `Designation`, `Department`,`Mobile_Number`,`Boarding_point`, `Route_no`, `Fee`, `Due`,`Verifiedby`,`Receipt_Number` FROM `faculty` ORDER BY Date_of_Payment DESC LIMIT ".$this_page_first_result.",".$results_per_page ;
	  $search_result1 = mysqli_query($conn,$st1 );
	 // echo $st1;
	  }
    ...
    <div id="printdiv">
      <div id="hidden_div">
            ...
            tr:nth-child(odd){background: rgba(17,110,176,0.2)} !important;"
			 width="80vw">
        <caption>Faculty Registrations</caption>
  ...
  <?php
   while($row = mysqli_fetch_array($search_result)){?>
    <tr>
		...
		<td class="wordbreak"><?php echo $row['Department'];?></td>
		<td><?php echo $row['Mobile_Number'];?></td>
		<td><?php echo $row['Boarding_point'];?></td>
  ...
  <?php } ?>
   ...
   <p style="color:#116EB0">Latest on Top</p>
      <table  class="table " style="text-align:center;border:1px solid black;margin-top:10px;"  width="95vw">
        <tr style=" background: rgba(17,110,176,0.9);color:white;">
		    <th>#</th>
			<th>Employee id</th>
			<th> Name </th>
