...
file name           : concession-report.php
database connection : buspassdatabase
table               : student
purpose             :
             The functionality of this page is to generate report based on a fee-concession
...
<title>Logistics Mgt. | Stud Concession Reports</title>
...
<p style="color:#116EB0;font-size:14px; "><a href="adminhome.php">Home > </a>Generate > Student Reports ><a href="concession-report.php"> Concession Report</a></p><br>
...
//  echo $_GET['date1'];
  ...
  if(isset($_GET['page']) )
  {
   ...
   // echo $dt1." ".$dt2;
      ...
      /* when date is picked */
       $query = "SELECT Receipt_Number,HallticketNumber,Name,Route_no,Boarding_point,Course,Department_Name,Year,Total,Due,Verifiedby,Remarks,`concession`,`cona_amount` FROM student where Date_of_Payment BETWEEN '$dt1' AND '".date('Y-m-d',strtotime('+1 day', strtotime($dt2)))."' AND concession = 'YES' ORDER BY Date_of_Payment DESC";
	 ...
	 /* when date is not picked and branch other than pharmacy is picked  */	
	  ...
	  {
		  $query = "SELECT Receipt_Number,HallticketNumber,Name,Route_no,Boarding_point,Course,Department_Name,Year,Total,Due,Verifiedby,Remarks,`concession`,`cona_amount` FROM student where concession = 'YES' ORDER BY Date_of_Payment DESC";
		  $search_result = mysqli_query($conn, $query);
	  }
    ...
    //echo $dt1." ".$dt2;
	   ...
	   $st1 = "SELECT * FROM student where Date_of_Payment BETWEEN '".$dt1."' AND '".date('Y-m-d',strtotime('+1 day', strtotime($dt2)))."' AND  concession = 'YES' ORDER BY Date_of_Payment DESC LIMIT ".$this_page_first_result.",".$results_per_page ;
	  ...
	  /* when date is not picked and branch other than pharmacy is picked  */
	 	...
	 	{
	  $st1 = "SELECT * FROM student where concession = 'YES' ORDER BY Date_of_Payment DESC LIMIT ".$this_page_first_result.",".$results_per_page ;
	  $search_result1 = mysqli_query($conn,$st1 );
	 // echo $st1;
	  }	 
  ...
  <?php
   while($row = mysqli_fetch_array($search_result)){?>
    <tr>
		...
		<td><?php echo $row['cona_amount'];?></td>
    </tr>
  <?php } ?>
